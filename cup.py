...
def bracket_table(final=None, semis=None, quarters=None):
	...
	def cell(y, x, m, winner=False, flip=False, top=False, bottom=False, left=False, right=False, gw=None):
		...
		if m is None:
			x1_style = x1_style.replace('right-align','center').replace('left-align','center')
			x2_style = x2_style.replace('right-align','center').replace('left-align','center')
	...
	if semis:
		grid[2][2] = f'<td class="w3-white w3-topbar w3-bottombar w3-border-black">v</td>'
		grid[2][8] = f'<td class="w3-white w3-topbar w3-bottombar w3-border-black">v</td>'
		...
		cell(2, 3, winner1, winner=True, top=True, bottom=True, right=True, gw=37) # S1-winner 2
		...
		cell(2, 6, winner2, winner=True, flip=True, top=True, bottom=True, left=True, gw=37) # S2-winner 3
		cell(2, 9, loser2, flip=True, top=True, bottom=True, right=True, gw=37) # S2-loser 4
		...
		grid[2][2] = f'<td class="w3-grey w3-topbar w3-bottombar w3-border-black">v</td>'
		grid[2][8] = f'<td class="w3-grey w3-topbar w3-bottombar w3-border-black">v</td>'
	...
	if final:
		grid[0][5] = f'<td class="w3-white w3-topbar w3-bottombar w3-border-black">v</td>'
		grid[1][3] = '|black'
		grid[1][6] = '|black'
		cell(0, 3, winner1, winner=winner1.get_event_score(38) > winner2.get_event_score(38), top=True, bottom=True, left=True, gw=38)
		cell(0, 6, winner2, winner=winner1.get_event_score(38) < winner2.get_event_score(38), flip=True, top=True, bottom=True, right=True, gw=38)
	
	else:
		grid[0][5] = f'<td class="w3-grey w3-topbar w3-bottombar w3-border-black">v</td>'
