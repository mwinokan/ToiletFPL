...
def weighted_average(
    this_season_by_gw,
    this_season_minutes_by_gw=None,
    last_season_total=None,
    last_season_minutes=None,
    plot=False,
    last_season_weight=1.0,
):
    """
    ...
    for i, (value, mins) in enumerate(
        zip(this_season_by_gw, this_season_minutes_by_gw, strict=True)
    ):
        if mins == 0:
            continue
    ...
    if plot:
        ...
        if last_season_total is not None:
            trace = go.Scatter(
                ...
                x=[n, n + 38],
                y=[
                    last_season_total / last_season_minutes,
                    last_season_total / last_season_minutes,
                ],
                mode="lines",
            )
