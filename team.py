...
class Team:
    ...
    def __init__(self, id, name, api, shortname=None):
        ...
        self._strength_overall_home = self.scale_strength(
            self._api._teamdata["strength_overall_home"][id - 1]
        )
        self._strength_overall_away = self.scale_strength(
            self._api._teamdata["strength_overall_away"][id - 1]
        )
        self._strength_attack_home = self.scale_strength(
            self._api._teamdata["strength_attack_home"][id - 1]
        )
        self._strength_attack_away = self.scale_strength(
            self._api._teamdata["strength_attack_away"][id - 1]
        )
        self._strength_defence_home = self.scale_strength(
            self._api._teamdata["strength_defence_home"][id - 1]
        )
        self._strength_defence_away = self.scale_strength(
            self._api._teamdata["strength_defence_away"][id - 1]
        )
    ...
    def expected_clean_sheet(self, opponent):
        if isinstance(opponent, list):
            ...
            for opp in opponent:
                p = (
                    self.clean_sheet_probability
                    + 1
                    - opp.clean_sheet_breaking_probability
                ) / 2
            ...
            prob = (
                self.clean_sheet_probability
                + 1
                - opponent.clean_sheet_breaking_probability
            ) / 2
    ...
    def difficulty_next5(self):
        if not self._difficulty_next5:
            ...
            for gw in range(
                self._api.current_gw + 1, min(self._api.current_gw + 5, 38)
            ):
                ...
                for fix, opp in zip(fixs, opps):
                    ...
                    total += self.strength(is_home, overall=True) - opp.strength(
