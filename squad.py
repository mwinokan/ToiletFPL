    ...
    def set_best_multipliers(self, gw, use_official=False, summary=False, debug=False):
        ...
        sorted_goalkeepers = sorted(
            ...
            key=lambda x: x.expected_points(
                gw=gw, debug=debug, use_official=use_official
            ),
            reverse=True,
        )
        ...
        if summary:
            for p in self.players:
                print(
                    f"{p.expected_points(gw=gw,debug=debug,use_official=use_official):5.1f}",
                    p.multiplier,
                    p.name,
                )

    def expected_transfer_return(
        self, gw, player_out, player_in, assume_best_multipliers
    ):
